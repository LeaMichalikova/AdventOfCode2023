fun pt1() {
    ...
    for (line in input.split('\n')) {
        ...
        resultSum += (num1.toString() + num2).toInt()
    }
    println(resultSum)
}
...
fun pt2() {
    ...
    for (line in input.split('\n')) {
        ...
        for (char in line) {
            if (char in '0'..'9') {
                numNum1 = char
                indexNum = line.indexOf(char)
                break
            }
        }
        for (num in numbers) {
            tempIndex = line.indexOf(num)
        ...
        if (index < indexNum && indexNum != 100) {
            num1 = (numbers.indexOf(num1)+1).toString()
        } else {
            num1 = numNum1.toString()
        }

        if (line == "2ninekvdbfnmjmd6ninentpktmgseven92") {
            println("pause lol")
        }
        ...
        for (char in line.reversed()) {
            if (char in '0'..'9') {
                numNum2 = char
                indexNum = line.lastIndexOf(char)
                break
            }
        }
        for (num in numbers) {
            tempIndex = line.lastIndexOf(num)
            if (tempIndex > index && tempIndex != -1) {
                num2 = num
                index = tempIndex
            }
        ...
        if (index > indexNum && index != 100) {
            num2 = (numbers.indexOf(num2)+1).toString()
        } else {
            num2 = numNum2.toString()
        }

        var temp = (num1 + num2).toInt()
        resultSum += temp
    }
    println(resultSum)
}
...
fun main() {
    /// one very ugly code that im ashamed of :D
    //pt1()
    pt2()
}


const val input = """51591twosix4dhsxvgghxq
425nine
llvmhjtr8nbbhrfone
