...
fun pt1_2() {
    ...
    for (line in input2.split('\n')) {
        var (gameNum, temp) = line.split(':')
        gameNum = gameNum.split(' ')[1]

        var gamePossible = true
        for (set in temp.split(';')) {
            for (idk in set.split(',')) {
                val one = idk.strip()
                val (number, colour) = one.split(' ')
                if (colour == "red" && number.toInt() > 12 ||
                    colour == "green" && number.toInt() > 13 ||
                    colour == "blue" && number.toInt() > 14) {
                    gamePossible = false
                    break
                }
            }
    ...
    for (line in input2.split('\n')) {
        ...
        for (set in temp.split(';')) {
            for (idk in set.split(',')) {
                val one = idk.strip()
                val (number, colour) = one.split(' ')
                if (colour == "red" && number.toInt() > minRed) {
                    minRed = number.toInt()
                } else if (colour == "green" && number.toInt() > minGreen) {
                    minGreen = number.toInt()
                } else if (colour == "blue" && number.toInt() > minBlue) {
                    minBlue = number.toInt()
                }
            }
            minGameSet = max(minGameSet, minRed * minBlue * minGreen)
        }
        //println(minGameSet)
        resultSum += minGameSet
    }
    println(resultSum)
}
...
const val input2 = """Game 1: 4 red, 1 green, 15 blue; 6 green, 2 red, 10 blue; 7 blue, 6 green, 4 red; 12 blue, 10 green, 3 red
